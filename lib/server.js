...
function CoAPServer(options, listener) {
  ...
  this._options.dtls = (options.dtls) ? true : false; // Might add other params later?
  if (this._options.dtls) {
    // If there a key / cert wae presented...
    this._options.dtls_cert = (options.dtls_cert) ? options.dtls_cert : false; // TODO: Might decide to generate one?
    this._options.dtls_key = (options.dtls_key) ? options.dtls_key : false; // TODO: Might decide to generate one?
  }
...
CoAPServer.prototype._sendError = function(payload, rsinfo, packet) {
  ...
  if (this._sock) {
    ...
    this._dtls_server.write(message);
  }
}
...
CoAPServer.prototype._sendProxied = function(packet, proxyUri, callback) {
  ...
  if (this._sock) {
    ...
    this._dtls_server.write(message, callback);
  }
}
...
CoAPServer.prototype.listen = function(port, address, done) {
  ...
  if (port == undefined) {
    port = (this._options.dtls) ? parameters.coapSecPort : parameters.coapPort
  ...
  if (this._options.dtls) {
    ...
    var _temp_opts = {
      ...
      key: this._options.dtls_key,
      handshakeTimeoutMin: 3000
        //cert: this._options.dtls_cert
    };
    this._dtls_server = dtls.createServer(_temp_opts);
    this._dtls_server.listen(port)
    this._sock = false // We don't have this in this case...

    this._dtls_server.on('secureConnection', function(client_sock) {
      console.log('New connection from ' + [client_sock.rinfo.address, client_sock.rinfo.port].join(':'));
      handleRequest(that)
    })
    console.log('Server using DTLS on port ', port, "\n");
  } else {
    // Non- DTLS workflow...
