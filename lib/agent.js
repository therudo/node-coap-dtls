...
var dtlsClient = require('node-mbed-dtls-client');
...
Agent.prototype._init = function initSock(dtlsOpts, newCallback) {
  ...
  console.log("INIT OPTS: " + dtlsOpts);
  if (dtlsOpts) {

    function clientSock(dtlsSock) {

      this.send = function(msg, offset, length, port, host, callback) {
        dtlsSock.write(msg, callback)
      }

      this.close = dtlsSock.close;
      ...
      this.address = function() {
        return {
          port: dtlsOpts.port,
          address: dtlsOpts.hostname,
          family: this._opts.type === 'udp4' ? 'IPv4' : 'IPv6'
        }
      }
    }

    console.log(JSON.stringify(dtlsOpts, 4));
    var bigSocket = dtlsClient.connect(dtlsOpts, (socket) => {
      socket.on('data', msg => {
        ...
        that._handle(msg, outSocket, outSocket)
      })

      that._sock = clientSock(socket);

      that._sock.on('error', function(err) {
